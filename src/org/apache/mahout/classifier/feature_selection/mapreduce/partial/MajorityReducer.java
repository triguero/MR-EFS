...
public class MajorityReducer extends MajorityIterativeReducer<StrataID,MapredOutput,StrataID,MapredOutput> {
  ...
  private static final Logger log = LoggerFactory.getLogger(FSReducer.class);
  ...
  */
  ...
  protected void cleanup(Context context) throws IOException, InterruptedException {
	 // log.debug("partition: {} numInstances: {}", partition, instances.size());
	    ...
	    log.info("reduce: {} numInstances: {}", strata, Majority.size());
	     ...
	     * 
	    if (!isNoOutput()) {
	    	MapredOutput emOut = new MapredOutput(Majority);
	        context.write(key, emOut);
	    }
	    
	    //save it in disk
	  //  Majority.save("prueba.dat");
